...
class Popup(db.Model):
    """Main popup configuration model"""
    ...
    popup_size = db.Column(db.String(20), default='medium')  # 'small', 'medium', 'large'
    ...
    def is_currently_active(self):
        """Check if popup should be shown based on dates"""
...
class CustomerConsent(db.Model):
    ...
    lead_id = db.Column(db.Integer, db.ForeignKey('popup_leads.id'), nullable=False)
    email = db.Column(db.String(255), nullable=False, index=True)
    
    # Consent Types: 'newsletter', 'terms_of_use', 'marketing_email'
