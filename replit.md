...
- **Ops Payment Flow (Cash / Card / Pay Later):** Post-order-create and orders-list checkout share the same flow. Top-level: Cash (change calculator + confirm), Credit Card (sub-actions: QR rendered inline, SMS to customer phone, Manual entry opening HYP hosted page in new tab), Pay Later (order stays `payment_status='pending'`). Orders list shows "ממתין לתשלום" pill on pending rows and a "סיום תשלום" button in the detail modal; the shared checkout modal supports ~600ms long-press on the Credit Card button to reveal sub-actions (quick tap defaults to QR). Endpoints: `POST /ops/api/orders/<id>/payment-qr` (generates HYP link if needed + base64 PNG QR), `GET /ops/api/orders/<id>/payment-status` (lightweight polling). While a QR/link is displayed, the client polls every 3s and auto-closes on paid.
...
- **Ops Orders Long-Press Quick Actions:** Swipe gestures on the Ops orders list have been removed in favor of a 500ms long-press (pointer events; ~10px move cancel; haptic vibrate) that opens a context-aware bottom action sheet (`#actionSheet` in `templates/ops/orders.html`). Sheet is built dynamically from row `data-*` attributes and uses event delegation (`data-as-action` / `data-as-id` / `data-as-extra`) — no inline `onclick` strings — and renders: a primary status action (Start Prep / Mark Ready / Out for Delivery / Mark Delivered) based on order status & type, a customer row (tel:/SMS/Print Bon), Waze navigation for delivery orders with an address, "סיום תשלום" when `payment_status='pending'`, secondary actions (Update Time / Open Detail), and a destructive Cancel. Quick tap continues to open the order detail modal; long-press click suppression prevents accidental double-trigger.
