...
- **SMS Notification Center:** Full SMS management system with admin CRUD at `/admin/sms-center`. Features: bilingual templates (`SMSTemplate` model with `name_he`, `content_he`, `template_type`, variable placeholders), auto-triggers (`SMSAutoTrigger` model linking order statuses to templates with optional branch scoping and duplicate prevention), message log viewer with filters (date, status, phone), quick-send form, and SMS provider health check. Ops integration: bottom-sheet modal in orders page for manual SMS with template insertion, auto-send on status change via `_auto_send_sms_for_status()`. All admin endpoints protected by `system.admin` RBAC. Provider: SMS4Free via `standalone_order_service/sms_helpers.py`.
...
- **Employee & PIN Management (Ops):** Module at `/ops/employees` for managing employee PINs from the iPad dashboard. Available to any user with `employees` permission (not superadmin-only). **Branch isolation:** Non-superadmin users only see employees from their own branch and cannot see/edit/toggle superadmin users. **Superadmin guard:** Non-superadmin users cannot create or promote users to superadmin — the superadmin checkbox is hidden in UI and blocked server-side. Non-superadmins are forced to create employees in their own branch only. Superadmins retain full access across all branches. Server-side validation includes branch existence check, permissions whitelist against `OPS_MODULES`, last-superadmin protection, self-toggle prevention, and cross-branch mutation blocking. `employees` added to `ManagerPIN.OPS_MODULES`.
- **Branch & Settings Isolation (Ops):** The `/ops/branches` module enforces branch-level isolation. Non-superadmin users with `branches` permission see only their own branch details, working hours, and toggles. Global settings (system-wide delivery/pickup) are hidden from non-superadmins both in UI and server-side. Branch toggle and working hours APIs enforce ownership checks — non-superadmins cannot modify settings for branches other than their own.
- **Database Schema:** Includes models for `FoodOrder`, `FoodOrderItem`, `MenuItemOptionGroup`, `MenuItemOptionChoice`, `ManagerPIN`, `Deal`, `Coupon`, `UpsellRule`, `OrderActivityLog`, `SMSLog`, `SMSTemplate`, `SMSAutoTrigger`, `EnrolledDevice`, `Branch`, `Role`, `Permission`, `AdminUser`, `Printer`, `PrinterStation`, `PrintStation`, `TimeLog`, `DeliveryZone`, `BranchMenuItem`.
